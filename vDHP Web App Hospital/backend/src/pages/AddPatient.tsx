...
const AddPatient = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ID will be generated by addPatient/updatePatient if not editing
      const id = editingId || ''; // Will be generated by patientService
      
      // Calculate age - use existing age if dateOfBirth is empty when editing
      let age: number;
      ...
      // Blood pressure - preserve existing if not provided when editing
      let bp: string;
      ...
      // Condition - preserve existing if medicalHistory is empty when editing
      ...
      // Last visit - preserve existing when editing (don't update to today)
      ...
      // Call backend to classify risk based on vitals (only if vitals are provided)
      ...
      if (formData.heartRate && formData.systolic && formData.diastolic && formData.temperature && formData.oxygenSat) {
        try {
          ...
          // If backend is unavailable, keep existing risk level or default 'low'
        }
      }

      // Vitals - preserve existing values if not provided when editing
      const vitals = {
      ...
      const payload = {
        id: editingId || '', // Will be generated by patientService if empty
        ...
        vitals,
        // Include medical history and related fields - preserve existing if empty when editing
      ...
      if (editingId) {
        // Update patient - this will sync to backend automatically
        await updatePatient(payload);
        alert(`Patient updated successfully!`);
      } else {
        addPatient(payload);
        // Try to sync to backend (non-blocking)
        ensurePatientInBackend(payload).catch((error) => {
          console.warn('Failed to sync patient to backend:', error);
          // Don't show error to user - patient is saved locally, backend sync is optional
        });
