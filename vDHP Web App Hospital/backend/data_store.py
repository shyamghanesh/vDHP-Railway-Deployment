...
try:
    ...
    import backend.sql_models as DB
...
class DataStore:
    ...
    def __init__(self):
        print(f"DEBUG: Initializing DataStore from {__file__}")
    ...
    def create_patient(self, patient_data: PatientCreate, doctor_id: str = None) -> PatientResponse:
        db = self._get_db()
        try:
            ...
            if patient_data.dateOfBirth:
                try:
                    birth_date = date.fromisoformat(patient_data.dateOfBirth)
                except ValueError:
                    birth_date = None
            ...
            gps = []
            if doctor_id:
                gps.append(Reference(reference=f"Practitioner/{doctor_id}"))
            ...
            fhir_patient = Patient(
                ...
                address=[address],
                telecom=telecoms,
                generalPractitioner=gps if gps else None
            )
            
            # Save to DB
            db_patient = DB.FhirPatient(
                id=pid,
                resource=json.loads(fhir_patient.json()),
                active=True,
                gender=fhir_patient.gender,
    ...
    def get_all_patients(self, doctor_id: str = None) -> List[PatientResponse]:
        db = self._get_db()
        try:
            ...
            for p in patients:
                try:
                    ...
                    if doctor_id:
                        is_match = False
                        if fhir_p.generalPractitioner:
                            for gp in fhir_p.generalPractitioner:
                                if gp.reference == f"Practitioner/{doctor_id}":
                                    is_match = True
                                    break
                        if not is_match:
                            continue
    ...
    def create_doctor(self, doctor_data: DoctorCreate) -> DoctorResponse:
        ...
        try:
            ...
            print(f"CRITICAL ERROR in create_doctor: {e}")
            import traceback
            traceback.print_exc()
            raise e
        finally:
    ...
    def _map_fhir_practitioner_to_response(self, res: Practitioner) -> DoctorResponse:
        name = "Unknown"
        if res.name:
            given = " ".join(res.name[0].given or [])
            family = res.name[0].family or ""
        ...
        try:
            ...
            for p in practitioners:
                try:
                    ...
                    fhir_p = Practitioner(**resource)
                    results.append(self._map_fhir_practitioner_to_response(fhir_p))
                except Exception as e:
                    print(f"Error loading doctor {p.id}: {e}")
                    import traceback
                    traceback.print_exc()
                    continue
            return results
    ...
    def create_provider(self, provider_data: ProviderCreate) -> ProviderResponse:
        ...
        try:
            ...
            if provider_data.email:
                org.telecom.append(ContactPoint(system='email', value=provider_data.email, use='work'))
            if provider_data.phone:
                org.telecom.append(ContactPoint(system='phone', value=provider_data.phone, use='work'))
            if provider_data.website:
                org.telecom.append(ContactPoint(system='url', value=provider_data.website, use='work'))
                
            # Add Address
            if provider_data.address:
                addr = Address(
                    line=[provider_data.address],
                    city=provider_data.city,
                    state=provider_data.state,
                    postalCode=provider_data.zipCode
                )
                org.address.append(addr)
            
            # Save to DB
            db_org = DB.FhirOrganization(
                id=pid,
                resource=json.loads(org.json()),
                active=True,
                name=provider_data.name
            )
            db.add(db_org)
    ...
    def get_provider_by_email(self, email: str) -> Optional[ProviderResponse]:
        """
        Find a provider (organization) by email.
        """
        db = self._get_db()
        try:
            # Scan organizations
            orgs = db.query(DB.FhirOrganization).all()
            for o in orgs:
                try:
                    resource = o.resource
                    if isinstance(resource, str):
                    ...
                    res = Organization(**resource)
                    # Check telecom for email
                    if res.telecom:
                        for t in res.telecom:
                            if t.system == 'email' and t.value == email:
                                # Found match!
                                addr = res.address[0] if res.address else None
                                return ProviderResponse(
                                    id=res.id,
                                    name=res.name,
                                    address=addr.line[0] if addr and addr.line else None,
                                    city=addr.city if addr else None,
                                    state=addr.state if addr else None,
                                    zipCode=addr.postalCode if addr else None,
                                    phone=next((t.value for t in res.telecom if t.system == 'phone'), None),
                                    email=email,
                                    website=next((t.value for t in res.telecom if t.system == 'url'), None),
                                    createdAt=datetime.now(timezone.utc)
                                )
                except Exception:
                    continue
    ...
    def _map_fhir_appointment(self, db_appt: DB.FhirAppointment) -> AppointmentResponse:
        resource = db_appt.resource
        if isinstance(resource, str):
            resource = json.loads(resource)
            
        return AppointmentResponse(
            id=db_appt.id,
            patientId=db_appt.patient_id,
            doctorId=db_appt.practitioner_id,
            scheduledDate=db_appt.start_date.date(),
            scheduledTime=db_appt.start_date.strftime("%H:%M"),
            reason=resource.get('description', ''),
            createdAt=db_appt.last_updated,
            updatedAt=db_appt.last_updated
        )

    def get_appointments_by_patient(self, patient_id: str) -> List[AppointmentResponse]:
        db = self._get_db()
        try:
            appts = db.query(DB.FhirAppointment).filter(DB.FhirAppointment.patient_id == patient_id).all()
            return [self._map_fhir_appointment(appt) for appt in appts]
        finally:
            db.close()
    ...
    def get_appointments_by_doctor(self, doctor_id: str) -> List[AppointmentResponse]:
        db = self._get_db()
        try:
            appts = db.query(DB.FhirAppointment).filter(DB.FhirAppointment.practitioner_id == doctor_id).all()
            return [self._map_fhir_appointment(appt) for appt in appts]
        finally:
            db.close()
    ...
    def get_all_appointments(self) -> List[AppointmentResponse]:
        db = self._get_db()
        try:
            appts = db.query(DB.FhirAppointment).all()
            return [self._map_fhir_appointment(appt) for appt in appts]
        finally:
            db.close()
    ...
    def get_appointment(self, appointment_id: str) -> Optional[AppointmentResponse]:
        db = self._get_db()
        try:
            appt = db.query(DB.FhirAppointment).filter(DB.FhirAppointment.id == appointment_id).first()
            if appt:
                return self._map_fhir_appointment(appt)
            return None
        finally:
            db.close()

    def update_appointment(self, appointment_id: str, status: str) -> Optional[AppointmentResponse]:
        db = self._get_db()
        try:
            appt = db.query(DB.FhirAppointment).filter(DB.FhirAppointment.id == appointment_id).first()
            if not appt:
            ...
            # Update DB status
            appt.status = status
            appt.last_updated = datetime.now(timezone.utc)
            ...
            # Update participant status based on overall status
            if 'participant' in resource:
                for p in resource['participant']:
                    if 'actor' in p and 'reference' in p['actor'] and 'Practitioner' in p['actor']['reference']:
                        if status == 'booked':
                            p['status'] = 'accepted' # Initially accepted by system? Or tentative?
                        elif status == 'cancelled':
                            p['status'] = 'declined'
                        else:
                            p['status'] = 'accepted'

            appt.resource = resource # Assign back to trigger update if needed (mostly for SQLAlchemy tracking)
    ...
    def create_medical_record(self, record_data: MedicalRecordCreate) -> MedicalRecordResponse:
        ...
        try:
            ...
            db_enc = DB.FhirEncounter(
                id=eid,
                resource=json.loads(encounter.json()),
                status='finished',
