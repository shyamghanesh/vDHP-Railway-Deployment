...
import { getAllAppointments, type Appointment } from '@/services/appointmentService';
  ...
  const loadDashboardData = React.useCallback(async () => {
    try {
      ...
      try {
        ...
        setRecentAppointments(appts.slice(0, 5)); // Show last 5
      } catch (error) {
      ...
      try {
        const docs = await getAllDoctorsFromBackend();
        setDoctors(docs);
      } catch (error) {
    ...
    <div className="min-h-screen bg-medical-bg">
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <Card className="bg-medical-card shadow-soft h-full">
            <CardHeader>
              <CardTitle className="text-medical-text text-lg">Recent Appointments</CardTitle>
            </CardHeader>
            <CardContent>
              {recentAppointments.length > 0 ? (
                <div className="space-y-4">
                  {recentAppointments.map((appt) => (
                    <div key={appt.id} className="flex items-center justify-between border-b border-border pb-2 last:border-0 last:pb-0">
                      <div>
                        {/*
                          Note: Create Appointment flow creates patient linkage but we might not have patient name
                          directly in the appointment object if backend doesn't populate it yet.
                          Ideally we'd lookup patient name from patients list.
                        */}
                        <p className="font-medium text-medical-text">
                          {patients.find(p => p.id === appt.patientId)?.name || 'Unknown Patient'}
                        </p>
                        <p className="text-xs text-medical-muted">{appt.scheduledDate} at {appt.scheduledTime}</p>
                        <p className="text-xs text-medical-muted truncate w-40">{appt.reason}</p>
                      </div>
                      <div className="text-right">
                        ...
                        >
                          {appt.status === 'confirmed' ? `Accepted by ${doctors.find(d => d.id === appt.doctorId)?.name || 'Doctor'}` :
                ...
                <p className="text-sm text-medical-muted text-center py-4">No recent appointments</p>
              )}
            </CardContent>
          </Card>
        </div>
